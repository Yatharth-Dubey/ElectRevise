...
export const View = () => {
  ...
  const [selectedJillaForAllotment, setSelectedJillaForAllotment] =
    useState("");
  ...
  useEffect(() => {
    const fetchVidhansabha = async () => {
      ...
      try {
        setLoading(true);
        const res = await axios.get(
          `${process.env.REACT_APP_API_URL}/getVidhansabhaList?jilla=${selectedJilla}`
        );
  ...
  useEffect(() => {
    const fetchMasterData = async () => {
      try {
        setLoading(true);
        const res = await axios.get(
          `${process.env.REACT_APP_API_URL}/getMasterData`
        );
  ...
  useEffect(() => {
    if (workerForm.jilla) {
      const filtered = masterData.filter(
        (item) => item.jilla === workerForm.jilla
      );
      setFilteredVidhansabhaList([
        ...new Set(
          filtered.map((item) => ({
            vidhansabha_no: item.vidhansabha_no,
            vidhansabha: item.vidhansabha,
          }))
        ),
      ]);
    } else {
  ...
  useEffect(() => {
    if (workerForm.jilla && workerForm.vidhansabha) {
      const filtered = masterData.filter(
        (item) =>
          item.jilla === workerForm.jilla &&
          item.vidhansabha === workerForm.vidhansabha
      );
      setFilteredTableList([
        ...new Set(
          filtered.map((item) => item.voter_table_name).filter(Boolean)
        ),
      ]);
      setFilteredNewVoterTableList([
        ...new Set(
          filtered.map((item) => item.new_voter_table_name).filter(Boolean)
        ),
      ]);
    } else {
      setFilteredTableList([]);
  ...
  const handleUpload = async () => {
    ...
    try {
      setLoading(true);
      await axios.post(
        `${process.env.REACT_APP_API_URL}/uploadMemberExcel`,
        formData,
        {
  ...
  const handleAddWorker = async () => {
    if (
      !workerForm.jilla ||
      !workerForm.vidhansabha_no ||
      !workerForm.vidhansabha ||
      !workerForm.booth_number ||
      !workerForm.worker_name ||
      !workerForm.mobile_no
    ) {
      return alert(
        "कृपया सभी आवश्यक फील्ड्स भरें! (जिला, विधानसभा नंबर, विधानसभा, बूथ नंबर, वर्कर नाम, मोबाइल नंबर)"
      );
    }
    ...
    try {
      setLoading(true);
      await axios.post(
        `${process.env.REACT_APP_API_URL}/addSingleWorker`,
        workerForm
      );
  ...
  const handleAllotment = async () => {
    try {
      ...
      const res = await axios.post(
        `${process.env.REACT_APP_API_URL}/allotTable`,
        {
  ...
  const saveEdit = async (sno) => {
    try {
      ...
      setTableData((prev) =>
        prev.map((row) => (row.sno === sno ? { ...row, ...editValues } : row))
      );
      setEditingRow(null);
    } catch (err) {
            ...
            <div className="worker-form-grid">
              ...
              <div className="form-group">
                ...
                <select
                  ...
                  onChange={(e) =>
                    handleWorkerFormChange("jilla", e.target.value)
                  }
                  className="register-upload"
                >
                  <option value="">-- जिला चुनें --</option>
              ...
              <div className="form-group">
                ...
                <select
                  ...
                  onChange={(e) =>
                    handleWorkerFormChange("vidhansabha_no", e.target.value)
                  }
                  className="register-upload"
              ...
              <div className="form-group">
                ...
                <select
                  ...
                  onChange={(e) =>
                    handleWorkerFormChange("vidhansabha", e.target.value)
                  }
                  className="register-upload"
                  disabled={!workerForm.jilla}
                >
              ...
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleWorkerFormChange("booth_number", e.target.value)
                  }
                  className="register-upload"
              ...
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleWorkerFormChange("worker_name", e.target.value)
                  }
                  className="register-upload"
              ...
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleWorkerFormChange("mobile_no", e.target.value)
                  }
                  className="register-upload"
              ...
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleWorkerFormChange("position", e.target.value)
                  }
                  className="register-upload"
              ...
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleWorkerFormChange("mandal_name", e.target.value)
                  }
                  className="register-upload"
              ...
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleWorkerFormChange("shakti_kshtra", e.target.value)
                  }
                  className="register-upload"
              ...
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleWorkerFormChange("personal_booth_no", e.target.value)
                  }
                  className="register-upload"
        ...
        {tableData.length > 0 && (
          <div className="table-wrapper">
            <table className="styled-table">
              <thead>
                <tr>
                  <th className="viewheading">SNO</th>
                  <th className="viewheading">Jilla</th>
                  <th className="viewheading">Vidhan No</th>
                  <th className="viewheading">Vidhansabha</th>
                  <th className="viewheading">Mandal Name</th>
                  <th className="viewheading">Shakti Kshtra</th>
                  <th className="viewheading">Personal Booth No</th>
                  <th className="viewheading">Personal Booth Name</th>
                  <th className="viewheading">Booth Number</th>
                  <th className="viewheading">Worker Name</th>
                  <th className="viewheading">Position</th>
                  <th className="viewheading">Mobile No</th>
                  <th className="viewheading">Allotted Table</th>
                  <th className="viewheading">Allotted NewVoter Table</th>
                  <th className="viewheading">Uploaded At</th>
                  <th className="viewheading">Edit</th>
                </tr>
              </thead>
              <tbody>
                {tableData.map((row) => (
