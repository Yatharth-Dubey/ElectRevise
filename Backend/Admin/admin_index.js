...
router.post("/sirmergedata", async (req, res) => {
  ...
  try {
    ...
    /* ---------------------- POLLING STATION MERGE ---------------------- */
      ...
      const [masterRows] = await con.query(
        "SELECT sir_voter_table_name FROM masterjilla WHERE jilla = ? AND vidhansabha = ?",
        [jilla, vidhansabha]
      );
      if (!masterRows.length || !masterRows[0].sir_voter_table_name) {
        return res.status(404).json({
          error: "Voter table not found for the specified jilla and vidhansabha",
        });
      }
      const sirvoterTableName = masterRows[0].sir_voter_table_name;
...
router.post("/createSirTable", async (req, res) => {
  try {
    let { jilla, vidhansabha_no, vidhansabha, tableName } = req.body;
    if (!jilla || !vidhansabha_no || !vidhansabha || !tableName) {
      return res.status(400).json({ message: "Missing fields" });
    }
    ...
    const clean = (str) =>
      str
        ...
        .replace(/__+/g, "_")
        .toLowerCase();
    const sirTableName = clean(tableName);
    if (!sirTableName) {
      return res.status(400).json({ message: "Invalid table name after cleaning" });
    }
    // SIR Table Schema (You can customize this if needed)
    const sirTableSchema = `
      sno INT AUTO_INCREMENT PRIMARY KEY,
      id VARCHAR(50),
      number VARCHAR(10),
      name VARCHAR(80),
    ...
    `;
    // Create SIR table
    await con.query(`CREATE TABLE IF NOT EXISTS \`${sirTableName}\` (${sirTableSchema})`);
    // Update masterjilla with SIR table reference
    const checkQuery = `
      SELECT * FROM masterjilla
      WHERE jilla = ? AND vidhansabha_no = ? AND vidhansabha = ?
    `;
    const [rows] = await con.query(checkQuery, [jilla, vidhansabha_no, vidhansabha]);
    if (rows.length > 0) {
      // Update existing record with SIR table name
      const updateQuery = `
        UPDATE masterjilla
        SET sir_voter_table_name = ?
        WHERE jilla = ? AND vidhansabha_no = ? AND vidhansabha = ?
      `;
      await con.query(updateQuery, [sirTableName, jilla, vidhansabha_no, vidhansabha]);
    } else {
      // Insert new record with SIR table name
      const insertQuery = `
        INSERT INTO masterjilla 
        (kshtra, jilla, vidhansabha_no, vidhansabha, sir_table_name)
        VALUES (?, ?, ?, ?, ?)
      `;
      await con.query(insertQuery, [jilla, jilla, vidhansabha_no, vidhansabha, sirTableName]);
    }
    return res.json({
      message: "SIR Table created & saved successfully!",
      sirTableName: sirTableName,
    });
  } catch (err) {
    console.error("SIR Table create error:", err);
    return res.status(500).json({ message: "Failed to create SIR table.", error: err.message });
  }
});
...
router.get("/getAllSirTables", async (req, res) => {
  try {
    const [rows] = await con.query(
      "SELECT DISTINCT sir_voter_table_name FROM masterjilla WHERE sir_voter_table_name IS NOT NULL"
    );
    console.log("SIR Tables:", rows);
    return res.json({ 
      status: true,
      message: "SIR tables fetched successfully",
      data: rows
    });
  } catch (err) {
    console.error("Fetch SIR tables error:", err);
    return res.status(500).json({ message: "Failed to fetch SIR tables." });
  }
});
...
router.get("/getSirPollingStations", async (req, res) => {
  try {
    let { jilla, vidhansabha } = req.query;
    if (!jilla || !vidhansabha) {
      return res.status(400).json({ error: "Jilla and Vidhansabha are required" });
    ...
    // Fetch all possible matching rows
    const [masterRows] = await con.query(
      "SELECT vidhansabha, sir_voter_table_name FROM masterjilla WHERE jilla = ?",
      [jilla]
    );
    if (masterRows.length === 0) {
      return res.status(404).json({ error: "No jilla found" });
    ...
    const sirvoterTableName = matchedRow.sir_voter_table_name;
    console.log("Matched Table:", sirvoterTableName);
    // Now get polling stations
    const [pollingRows] = await con.query(
      `SELECT DISTINCT polling_station FROM ?? WHERE 1 ORDER BY polling_station`,
      [sirvoterTableName]
...
router.post("/allotTable", async (req, res) => {
  try {
    ...
    // Jilla-wise query - agar jilla hai toh filter lagao
...
function generatePasskeyForJilla(jilla) {
  ...
  const num = Math.floor(Math.random() * 900) + 100; // 100-999 (3 digits)
  return `${base}-${num}`;
}
...
router.post('/uploadJillaLogin', upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    // expect columns: kshtra, jilla, ass no., vidhansabha
    // extract unique jilla values (non-empty)
    ...
    try {
      ...
      for (const jilla of jillas) {
        ...
        // generate unique passkey (retry loop if collision)
        let attempts = 0;
        let passkey = null;
        while (attempts < 10) {
