...
const SECRET_KEY = "Sachin321";
router.post("/master", async (req, res) => {
  try {
    const { mobile_no } = req.body;
    console.log("Received number:", mobile_no);
    if (!mobile_no) {
    ...
    const otp = 1111;
    if (mobile_no === "1234567899") {
      await con.query("INSERT INTO registerdata (number, otp) VALUES (?, ?)", [
        mobile_no,
        otp,
      ]);
      ...
      return res.json({
        status: true,
        message: "OTP sent successfully (master bypass)!",
      });
    }
    const [result] = await con.query(
      "SELECT * FROM master_new WHERE mobile_no = ?",
      [mobile_no]
    );
    if (result.length === 0) {
    ...
    console.error("❌ Error in /master:", err);
...
router.post("/addNewSirVoter", verifyToken, async (req, res) => {
  try {
    ...
    // Agar mukhiya / address missing hai
    if (!mukhiya || !address) {
      return res.status(400).json({
        status: false,
        error: "Mukhiya and Address are required",
    ...
    // Pehle family_id nikal lo (agar pehle se exist hai to reuse)
    const checkQuery = `
      SELECT family_id 
      FROM ${allotted_newvoter_table_name} 
      WHERE mukhiya = ? AND address = ? 
    ...
    // 🔹 SINGLE VOTER MODE (old compatibility) – jab `voters` array nahi aata
    const singleVoterMode = !voters;
    ...
    if (singleVoterMode) {
      ...
      // sirTable + main table update
      await con.query(
        `UPDATE ${sirTable} SET status='completed', family_id=? WHERE id=?`,
        [finalFamilyId, id]
      );
      await con.query(
        `UPDATE ${allotted_table_name} SET status='completed', family_id=? WHERE id=?`,
    ...
    // 🔹 MULTI-VOTER MODE (jo hum frontend se use kar rahe hain)
    if (!Array.isArray(voters) || voters.length === 0) {
      return res.status(400).json({
        status: false,
        message: "No voters data received",
    ...
    // 1) Voters ko do group me tod do: extra vs normal
    // Frontend ne har voter ke andar `isFromExtra: true/false` bhejna hai
    const extraVoters = voters.filter((v) => v.isFromExtra);
    const normalVoters = voters.filter((v) => !v.isFromExtra);

    console.log("Normal voters count:", normalVoters.length);
    console.log("Extra voters count:", extraVoters.length);

    // 2) Sabhi voters (normal + extra) ko allotted_newvoter_table_name me insert karo
    const insertMultiQuery = `
      INSERT INTO ${allotted_newvoter_table_name} (
        number,id,name,father,house,age,gender,polling_station,
        station_address,kinType,year,date1,date2,add1,add2,postcode,
    ...
    // 3) NORMAL voters ke liye: sirTable me status='completed'
    const normalIds = normalVoters.map((v) => v.id);

    if (normalIds.length > 0) {
      await con.query(
        `UPDATE ${sirTable} SET status='completed', family_id=? WHERE id IN (?)`,
        [finalFamilyId, normalIds]
      );
    }

    // 4) EXTRA voters ke liye: sirTable me INSERT karo, status='sir'
    if (extraVoters.length > 0) {
      const insertSirExtraQuery = `
        INSERT INTO ${sirTable} (
          id,number,name,father,house,age,gender,polling_station,
          station_address,kinType,year,date1,date2,add1,add2,postcode,
          booth_number,add1_number,add2_number,status,family_id
        )
        VALUES ?
      `;

      const sirExtraValues = extraVoters.map((v) => [
        ...
        v.add1_number || "",
        v.add2_number || "",
        "sir",             // ✅ yahan status 'sir'
        finalFamilyId,
      ]);

      await con.query(insertSirExtraQuery, [sirExtraValues]);
    }

    // console.log(
    //   "Data Fetched -> familyId:",
    //   finalFamilyId,
    //   "normalIds:",
    //   normalIds,
    //   "extraIds:",
    //   extraVoters.map((v) => v.id)
    // );

    // 5) Voter count update (tumhara purana logic)
    await updateVoterCount(
      voters[0].booth_number,
      voters[0].add1_number,
      voters.length
    );
...
router.get("/getVoterData", verifyToken, async (req, res) => {
  ...
  const q = `SELECT * FROM ${allotted_table_name} WHERE add1_number = ? AND booth_number = ?`;
  console.log("query->",q);
